...
## Итог
Пользователь подтвердил направление продукта: локальная настольная игра `Сапёр` на Python. Первый предметный pass должен быть открыт отдельно и может включать product/technical docs и реализацию MVP.
